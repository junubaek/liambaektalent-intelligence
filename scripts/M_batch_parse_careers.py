...
def main():
    ...
    print("Fetching valid candidates from SQLite...")
    c.execute("SELECT id, name_kr, raw_text FROM candidates WHERE is_duplicate=0")
    targets = c.fetchall()
    
    # Filter targets to only those without parsed_career_json
    to_parse = []
    for row in targets:
        cid = row[0]
        ...
        if "parsed_career_json" in cinfo and cinfo["parsed_career_json"]:
            continue
    ...
    total = len(to_parse)
    print(f"Found {total} candidates needing detailed career parsing.")
    ...
    for i, (cid_str, name, raw_text) in enumerate(to_parse):
        ...
        try:
            ...
            print(f"[{i+1}/{total}] Failed to parse {name} ({cid_str}): {e}", flush=True)
            fail += 1
            
        time.sleep(0.1) # Small sleep to prevent aggressive rate limits
    ...
    print(f"Batch parse fully completed! Success: {success}, Failed: {fail}")
