...
for query, seniority, target_id, name in tests:
    ...
    cur.execute("SELECT name_kr, total_years, sector, is_duplicate, duplicate_of FROM candidates WHERE id=?", (target_id,))
    row = cur.fetchone()
    if row:
        c_name, c_years, c_sector, is_dup, dup_of = row
        primary_sector = c_sector.split(',')[0].strip() if c_sector else ''
        print(f"  Candidate Sector: {primary_sector}, Candidate Years: {c_years}, is_duplicate: {is_dup}, duplicate_of: {dup_of}")
        ...
        print("  Candidate NOT FOUND in SQL!")
