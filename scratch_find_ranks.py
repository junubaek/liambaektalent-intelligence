...
for query, seniority, name, target_id in tests:
    ...
    for idx, c in enumerate(matched):
        if c.get('id') == target_id:
            print(f'[{name}] Found at rank {idx+1}: score={c.get("final_score")}, g={c.get("g_score")}, v={c.get("v_score")}')
            found = True
            break
    if not found:
        print(f'[{name}] NOT found in matched list (total matched: {len(matched)})')
