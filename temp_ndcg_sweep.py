...
with open('temp_rrf_patcher.py', 'r', encoding='utf-8') as f:
    text = f.read()
...
for name, top_v, top_g in combinations:
    ...
    patched = re.sub(r"CALL db\.index\.vector\.queryNodes\('candidate_embedding', \d+, \$queryVector\)", 
                     f"CALL db.index.vector.queryNodes('candidate_embedding', {top_v}, $queryVector)", text)
                     
    patched = re.sub(r"graph_results = graph_candidates\[:\d+\]", 
                     f"graph_results = graph_candidates[:{top_g}]", patched)
                     
    with open('temp_rrf_patcher.py', 'w', encoding='utf-8') as f:
        f.write(patched)
        
    subprocess.run(["python", "-X", "utf8", "temp_rrf_patcher.py"], check=True)
