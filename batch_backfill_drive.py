...
def main():
    print("Starting Google Drive URL Backfill...")
    db = SessionLocal()
    notion = HeadhunterDB()
    ...
    try:
        ...
        for i, (cid, name) in enumerate(targets):
            ...
            try:
                ...
                # Check for drive URL in any URL / Files property
                drive_url = None
                for prop_name, prop_data in props.items():
                    ptype = prop_data.get("type", "")
                    if ptype == "url":
                        val = prop_data.get("url")
                        if val and "drive.google.com" in val:
                            drive_url = val
                            break
                    elif ptype == "files":
        ...
        print(f"\n✨ Backfill Complete! Newly acquired: {success_count} | Still missing: {no_url_count}")
