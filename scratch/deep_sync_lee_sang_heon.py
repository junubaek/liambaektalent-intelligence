...
"""
    try:
        res = gemini.get_chat_completion_json(prompt, model="gemini-3.0-flash-preview")
        if not res: return
        ...
        cur.execute("""
            UPDATE candidates 
            SET profile_summary = ?, sector = ?, seniority = ?, is_neo4j_synced = 1
            WHERE id = ?
        """, (summary, sector, seniority, candidate_id))
        conn.commit()
