...
def run_resync_all_remaining():
    print(f"Starting Final Sync for all unsynced candidates...")
    ...
    cur.execute("""
        ...
        WHERE is_duplicate = 0
          AND (is_neo4j_synced = 0 OR is_neo4j_synced IS NULL)
          AND is_parsed = 1
    """)
    targets = cur.fetchall()
    
    print(f"Retrieved {len(targets)} candidates needing sync.")
    ...
    with driver.session() as session:
        for i, row in enumerate(targets):
            ...
            print(f"[{i+1}/{len(targets)}] Syncing: {name}")
    ...
    conn.close()
    driver.close()
    print(f"\nFinal Sync Finished. Success: {success}, Fail: {fail}")

if __name__ == "__main__":
    run_resync_all_remaining()
