...
def calculate_lee_ndcg():
    dataset_path = 'golden_dataset_v10.json'
    with open(dataset_path, 'r', encoding='utf-8') as f:
        d = json.load(f)

    lee_queries = ["General Affairs Manager", "Overseas Sales Manager", "Bioinformatics Engineer"]
    d = [item for item in d if (item.get('query') or item.get('jd_query')) in lee_queries]
    ...
    scores = []
    print(f"=== Lee Sang-heon NDCG Evaluation ===")
    ...
    for q, info in query_targets.items():
        ...
        print(f'{score:.4f} | hits:{hits}/{len(relevant_ids)} | {q}')
        for i, cand in enumerate(matched[:10]):
            cid = str(cand.get('id', '')).lower()
            rel = relevance_scores.get(cid, 0.0)
    ...
    if scores:
        ...
        print(f"AVERAGE LEE NDCG: {avg_ndcg:.4f}")
