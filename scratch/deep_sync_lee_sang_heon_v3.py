...
def parse_and_sync(candidate_id):
    ...
    # 1. Embedding (For Neo4j Vector Index)
    print(f"  Calculating embedding...")
    embedding = openai.embed_content(text[:8000]) # text-embedding-3-small
    
    # 2. Parsing (For Summary & Skills)
...
"""
    try:
        ...
        with driver.session() as session:
            # Update node including embedding
            session.run("""
